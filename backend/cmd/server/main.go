...
func main() {
	...
	chargeRepo := repository.NewSessionChargeRepository(db)
	flowRepo := repository.NewWalletFlowRepository(db)
	...
	stationService := service.NewStationService(stationRepo, db, logger)
	...
	sessionService := service.NewSessionService(sessionRepo, chargeRepo, flowRepo, stationService, userPkgRepo, userRepo, reservationRepo, db, logger)
	// 使用中机位标记故障时，在机位状态事务内中断上机并退费（避免 service 间循环依赖，使用回调注入）。
	stationService.SetFaultInterrupter(sessionService)
